 ...
 */
export const typeTextToSchema = (typeText: string): TSchema => {
  ...
  const schema = convertNodeToSchema(typeNode) as any; // Temporarily using 'any' to modify $id
  schema.$id = typeName;
 ...
 * @returns The corresponding TypeBox schema.
 */
const convertNodeToSchema = (node: ts.TypeNode): TSchema => {
  switch (node.kind) {
    ...
    case ts.SyntaxKind.ArrayType:
      return Type.Array(
        convertNodeToSchema((node as ts.ArrayTypeNode).elementType)
      );
    case ts.SyntaxKind.TypeLiteral:
      const members = (node as ts.TypeLiteralNode).members;
      const properties: Record<string, TSchema> = {};
      let additionalProperties: TSchema | undefined;
      ...
      members.forEach((member) => {
        if (
          ...
          if (description) {
            (value as any).description = description; // Temporarily using 'any' to modify description
          }
          ...
          if (keyType && valueType) {
            ...
            if (keyType.kind === ts.SyntaxKind.StringKeyword) {
              ...
              throw new Error(
                "Only string keys are supported in index signatures."
              );
            }
          }
        }
      ...
      if (additionalProperties) {
        (objectSchema as any).additionalProperties = additionalProperties;
      }

      return objectSchema;
    case ts.SyntaxKind.UnionType:
      return Type.Union(
        (node as ts.UnionTypeNode).types.map((typeNode) =>
          convertNodeToSchema(typeNode)
        )
      );
    case ts.SyntaxKind.LiteralType: {
      ...
      throw new Error(
        `Unsupported literal type: ${ts.SyntaxKind[literalNode.literal.kind]}`
      );
    }
 ...
 */
const getCommentForNode = (node: ts.Node): string | undefined => {
